...
struct row createRowPro_bt(int numberOfEdges) {
	int row_id, edge_id;
	struct row *newRow;
...
struct row *createArrayPro_bt(int numOfElements) {
	int i, j, edge_id;
    struct row *array;
	...
	for(i = 0; i< numOfElements; i++)
		array[i] = createRowPro_bt(numOfElements);
 ...
 * CHILD PROCESS CALL
 */
void child_labor_bt(shared_memory *mem_ptr, int fork_count, int k) {
	int rows, columns;
...
void processWarshall_bt(shared_memory *mem_ptr) {
	...
	for (k = 0; k < mem_ptr->numEdges; k++) {
		...
		for (fork_count = 0; fork_count < mem_ptr->numProcess; fork_count++) {
			...
			if ((pid_list = fork()) == 0) {
				...
				child_labor_bt(mem_ptr, fork_count, k);
			} else {
...
void warshallsProcessed_bt(struct row* boolMatrix, struct row* warPath, int numEdges, int numProcess) {
    ...
    mem_ptr->edge_ptr = createArrayPro_bt(numEdges);
	...
	processWarshall_bt(mem_ptr);
	//threadedWarshall(mem_ptr);
	printf("Final Graph\n");
